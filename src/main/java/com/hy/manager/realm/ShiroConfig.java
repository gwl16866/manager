...
import com.hy.manager.filter.CustomRolesOrAuthorizationFilter;
import com.hy.manager.filter.CustomSessionGenerator;
import com.hy.manager.filter.CustomSessionManager;
...
public class ShiroConfig {
    ...
    public ShiroFilterFactoryBean shiroFilter(SecurityManager securityManager) {
        ...
        shiroFilterFactoryBean.setLoginUrl("/login/needLogin");

        //登录成功，跳转url，如果前后端分离，则没这个调用
        shiroFilterFactoryBean.setSuccessUrl("/");

        //没有权限，未授权就会调用此方法， 先验证登录-》再验证是否有权限
        shiroFilterFactoryBean.setUnauthorizedUrl("/login/needPerms");
...
//    }
    ...
    public RedisManager getRedisManager() {
        ...
        redisManager.setPassword("huayu");
        return redisManager;
    }
     ...
     */
    @Bean
    @DependsOn("lifecycleBeanPostProcessor")  //依赖这个bean
